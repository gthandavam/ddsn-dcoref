...
def objective_function(curr_state, next_state, prev_path, prob_matrix):
  ...
  for i in xrange(len(prev_path[curr_state])):
    ret += prob_matrix[prev_path[curr_state][i]][next_state]
...
def get_viterbi_path(prob_matrix):
  '''
  Return the (max) viterbi path given the log prob matrix
  '''
  ...
  for i in xrange(n):
    if i== 0:
      V_dp_table[0][i] = 0 #log(1) == 0 #initializing for first observation, starting with 0
    else:
      V_dp_table[0][i] = math.log(0.0001) #giving other states a smoothing probability
    path[i] = [i]
  ...
  for t in xrange(1, n):
    ...
    for y in xrange(n):
      ...
      candidates = [(V_dp_table[t-1][y0] + objective_function(y0, y, path, prob_matrix), y0) for y0 in xrange(n)]
      if(len(candidates) != 0):
...
if __name__ == '__main__':
 ...
 [ -6.77236809e-05,  -2.37418621e-02,  -2.94629636e-01,  -6.26100265e-02,
   -2.61502056e-01,  -1.29462895e-01,  -6.40501774e-02,  -2.37732884e+00,
    math.log(0.0001),  -2.55088289e+00,  -4.13926410e-02,  -1.69683488e+00,],
 [ -8.95783518e-07,  -5.55074940e-04,  -8.11272468e-03,  -3.12148255e-03,
   -7.07369657e-03,  -3.26563689e-03,  -3.19571826e-03, -1.21407275e-01,
   -8.12238942e-02,   math.log(0.0001),  -5.01987892e-03,  -2.74875574e-01,],
 [ -3.12266384e-04,  -1.44134954e-01,  -1.18116596e+00,  -1.03702043e+00,
   -1.08850438e+00,  -6.47602885e-01,  -1.05223234e+00,  -3.82880113e+00,
   -3.20527710e+00,  -5.29685836e+00,   math.log(0.0001),  -4.33464783e+00,],
 [ -4.96644043e-06,  -1.47264141e-03,  -2.16187455e-02,  -2.92739823e-02,
   -1.88418914e-02,  -8.68073381e-03,  -2.99741045e-02,  -5.40314436e-01,
   -2.02437723e-01,  -1.42572832e+00,  -1.31931369e-02,   math.log(0.0001)]]

  print get_viterbi_path(wt_m)

  wt_m = [[math.log(0.0001), math.log(0.7), math.log(0.2)], [math.log(0.0001), math.log(0.0001), math.log(0.6)], [math.log(0.1), math.log(0.1), math.log(0.2)]]

  print get_viterbi_path(wt_m)

  wt_m_1 = [[math.log(0.0001),math.log(0.0001),0,math.log(0.0001)], [0,math.log(0.0001),math.log(0.0001),math.log(0.0001)], [math.log(0.0001),math.log(0.0001),math.log(0.0001),0], [math.log(0.0001),0,math.log(0.0001),math.log(0.0001)]]
